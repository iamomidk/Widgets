...
class TextView : MaterialTextView {
	...
	private fun initialize(
		...
		if (attrs != null) {
			context.obtainStyledAttributes(attrs, R.styleable.TextView).apply {
				if (hasValue(R.styleable.TextView_fontStyle)) {
					when (FontSTYLE.values()[getInt(R.styleable.TextView_fontStyle, 0)]) {
