...
class AutoCompleteTextView : MaterialAutoCompleteTextView {
	...
	private fun initialize(attrs: AttributeSet?) {
		...
		if (attrs != null) {
			context.obtainStyledAttributes(attrs, R.styleable.AutoCompleteTextView).apply {
				if (hasValue(R.styleable.AutoCompleteTextView_fontStyle)) {
					when (FontSTYLE.values()[getInt(R.styleable.AutoCompleteTextView_fontStyle, 0)]) {
